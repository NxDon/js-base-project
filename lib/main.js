 ...
 * @returns {*}
 */
function concatPostNum(array, result) {
    array.forEach((barcode) => {
        result = result.concat(barCodeArr.indexOf(barcode));
    });
    return result;
}

/**
 * get a string of barcode return a string of post number
 * @param realBarcode
 * @returns {string}
 */
function mapBarcodeToNumber(realBarcode) {
    let result = "";
    let barcode = splitBarcodeToArr(realBarcode);
    result = concatPostNum(barcode, result);
    return result;
}
...
function postalNumericDecode(barcode) {
    ...
    if (barcode.length === 32) {
        ...
        return mapBarcodeToNumber(realBarcode);
    }
